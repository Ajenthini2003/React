...
export default function Student() {
  ...
  return (
    <>
      <div style={{ backgroundColor: "#c2ab80ff", marginLeft: "100px",marginRight:"100px", color: "#800000", padding: "20px", borderRadius:"10px" ,height:"810px",marginTop:"10px"}}>
        
        
        <input
          style={{ marginLeft: "100px", marginTop:"-40px",width:"400px" ,height:"25px",borderRadius:"10px"}}
          type="text"
        ...
        <table cellPadding="8" style={{ marginTop: "20px",marginLeft:"50px",height:"500px", borderCollapse: "collapse", backgroundColor: "white", border: "1px solid #800000", width:"1600px" }}>
          <thead>
            <tr>
              {fieldNames.map((field) => (
                <th key={field} style={{ border: "1px solid #800000", padding:"8px" }}>{field.toUpperCase()}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                <tr key={index}>
                  {fieldNames.map((field) => (
                    <td key={field} style={{ border: "1px solid #800000", padding:"8px" }}>{item[field]}</td>
                  ))}
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={fieldNames.length} style={{ textAlign: "center", padding:"8px" }}>
